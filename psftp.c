...
#define sftp_log printf
#else
#include "terminal.h"
int __sftp_log(sftp_handle* sftp, const char* fmt, ...)
{
    if (!sftp->log_handle) return -1;
    ...
    for (; (i < 1024) && (buffer[i]); i++){
        ...
        {
            buffer_handled[j++] = buffer[i];
        }
    }
    buffer_handled[j++] = 0;
    term_data((Terminal*)sftp->log_handle, 1, buffer_handled, strlen(buffer_handled));
    return 0;
}
#define sftp_log(fmt,...) __sftp_log(sftp, fmt, ##__VA_ARGS__)
#endif /*TAB*/
...
static int psftp_connect(sftp_handle* sftp, char *userhost, char *user, int portnumber);
static int do_sftp_init(sftp_handle* sftp);
 ...
 */
char *canonify(sftp_handle* sftp, char *name)
{
    ...
    assert(rreq == req);
    canonname = fxp_realpath_recv(sftp, pktin, rreq);
	 ...
	 * then put the final component sftp->back on.
	 * 
	 * Special cases:
	 * 
...
static void not_connected(sftp_handle* sftp)
{
    sftp_log("psftp: not connected to a host; use \"open host.name\"\n");
}

/* ----------------------------------------------------------------------
 * The meat of the `get' and `put' commands.
 */
int sftp_get_file(sftp_handle* sftp, char *fname, char *outfname, int recurse, int restart)
{
	...
	rreq = sftp_find_request(sftp, pktin = sftp_recv(sftp));
	assert(rreq == req);
	result = fxp_stat_recv(sftp, pktin, rreq, &attrs);
	...
	if (result &&
		...
		sftp_log("%s: Cannot create directory\n", outfname);
		return 0;
	    }
	    ...
	    if (!dirhandle) {
		sftp_log("%s: unable to open directory: %s\n",
		...
		if (names == NULL) {
			...
			break;
		    sftp_log("%s: reading directory: %s\n", fname, fxp_error(sftp));
		    sfree(ournames);
		    return 0;
		}
		if (names->nnames == 0) {
		...
		for (i = 0; i < names->nnames; i++)
			...
			if (!vet_filename(names->names[i].filename)) {
			    sftp_log("ignoring potentially dangerous server-"
				   "supplied filename '%s'\n",
				   names->names[i].filename);
			} else {
			    ournames[nnames++] =
				fxp_dup_name(sftp, &names->names[i]);
			}
		    }
		fxp_free_names(sftp, names);
	    }
		...
		if (outfname)
		    ...
		    nextoutfname = dupstr(ournames[i]->filename);
		ret = sftp_get_file(sftp, nextfname, nextoutfname, recurse, restart);
		restart = FALSE;       /* after first partial file, do full */
		sfree(nextoutfname);
		sfree(nextfname);
		if (!ret) {
		    for (i = 0; i < nnames; i++) {
			fxp_free_name(sftp, ournames[i]);
		    }
		    sfree(ournames);
		    return 0;
		}
    ...
    sftp_register(sftp, req = fxp_open_send(sftp, fname, SSH_FXF_READ));
    rreq = sftp_find_request(sftp, pktin = sftp_recv(sftp));
    assert(rreq == req);
    fh = fxp_open_recv(sftp, pktin, rreq);

    if (!fh) {
	sftp_log("%s: open for read: %s\n", fname, fxp_error(sftp));
	return 0;
    }
    ...
    if (!file) {
	sftp_log("local: unable to open %s\n", outfname);

	sftp_register(sftp, req = fxp_close_send(sftp, fh));
	rreq = sftp_find_request(sftp, pktin = sftp_recv(sftp));
	assert(rreq == req);
	fxp_close_recv(sftp, pktin, rreq);
	...
	if (seek_file(file, uint64_make(0,0) , FROM_END) == -1) {
	    close_wfile(file);
	    sftp_log("reget: cannot restart %s - file too large\n",
		   outfname);
	    sftp_register(sftp, req = fxp_close_send(sftp, fh));
	    rreq = sftp_find_request(sftp, pktin = sftp_recv(sftp));
	    assert(rreq == req);
	...
	sftp_log("reget: restarting at file position %s\n", decbuf);
    } else {
	offset = uint64_make(0, 0);
    }

    sftp_log("remote:%s => local:%s\n", fname, outfname);
     ...
     */
    ret = 1;
    xfer = xfer_download_init(sftp, fh, offset);
    while (!xfer_done(sftp, xfer)) {
	void *vbuf;
	int ret, len;
	int wpos, wlen;
	...
	ret = xfer_download_gotpkt(sftp, xfer, pktin);

	if (ret < 0) {
	    if (!shown_err) {
		sftp_log("error while reading: %s\n", fxp_error(sftp));
		shown_err = TRUE;
	    }
            ret = 0;
	}

	while (xfer_download_data(sftp, xfer, &vbuf, &len)) {
		...
		if (wlen <= 0) {
		    sftp_log("error while writing local file\n");
		    ret = 0;
...
int sftp_put_file(sftp_handle* sftp, char *fname, char *outfname, int recurse, int restart)
{
    struct fxp_handle *fh;
    struct fxp_xfer *xfer;
    struct sftp_packet *pktin;
	 ...
	 * unless it already exists.
	 */
	sftp_register(sftp, req = fxp_stat_send(sftp, outfname));
	rreq = sftp_find_request(sftp, pktin = sftp_recv(sftp));
	assert(rreq == req);
	result = fxp_stat_recv(sftp, pktin, rreq, &attrs);
	if (!result ||
	    !(attrs.flags & SSH_FILEXFER_ATTR_PERMISSIONS) ||
	    !(attrs.permissions & 0040000)) {
	    sftp_register(sftp, req = fxp_mkdir_send(sftp, outfname));
	    rreq = sftp_find_request(sftp, pktin = sftp_recv(sftp));
	    assert(rreq == req);
	    result = fxp_mkdir_recv(sftp, pktin, rreq);

	    if (!result) {
		sftp_log("%s: create directory: %s\n",
		       outfname, fxp_error(sftp));
		return 0;
	    }
	}
	...
	if (!dh) {
	    sftp_log("%s: unable to open directory\n", fname);
	    return 0;
	}
	...
	i = 0;
        if (restart) {
            while (i < nnames) {
                ...
                sftp_register(sftp, req = fxp_stat_send(sftp, nextoutfname));
                rreq = sftp_find_request(sftp, pktin = sftp_recv(sftp));
                assert(rreq == req);
                result = fxp_stat_recv(sftp, pktin, rreq, &attrs);
                sfree(nextoutfname);
	...
	for (; i < nnames; i++) {
	    ...
	    nextoutfname = dupcat(outfname, "/", ournames[i], NULL);
	    ret = sftp_put_file(sftp, nextfname, nextoutfname, recurse, restart);
	    restart = FALSE;	       /* after first partial file, do full */
	    sfree(nextoutfname);
	    sfree(nextfname);
	    if (!ret) {
    ...
    if (restart) {
	sftp_register(sftp, req = fxp_open_send(sftp, outfname, SSH_FXF_WRITE));
    } else {
	sftp_register(sftp, req = fxp_open_send(sftp, outfname, SSH_FXF_WRITE |
	...
	sftp_register(sftp, req = fxp_fstat_send(sftp, fh));
	rreq = sftp_find_request(sftp, pktin = sftp_recv(sftp));
	assert(rreq == req);
	ret = fxp_fstat_recv(sftp, pktin, rreq, &attrs);

	if (!ret) {
	    close_rfile(file);
	    sftp_log("read size of %s: %s\n", outfname, fxp_error(sftp));
	    return 0;
	}
	if (!(attrs.flags & SSH_FILEXFER_ATTR_SIZE)) {
	    close_rfile(file);
	    sftp_log("read size of %s: size was not given\n", outfname);
	    return 0;
	}
	offset = attrs.size;
	uint64_decimal(offset, decbuf);
	sftp_log("reput: restarting at file position %s\n", decbuf);
     ...
     */
    ret = 1;
    xfer = xfer_upload_init(sftp, fh, offset);
    err = eof = 0;
    while ((!err && !eof) || !xfer_done(sftp, xfer)) {
	char buffer[4096];
	int len, ret;

	while (xfer_upload_ready(sftp, xfer) && !err && !eof) {
	    len = read_from_file(file, buffer, sizeof(buffer));
	    if (len == -1) {
		sftp_log("error while reading local file\n");
		err = 1;
	    } else if (len == 0) {
		eof = 1;
	    } else {
		xfer_upload_data(sftp, xfer, buffer, len);
	    }
	}

	if (!xfer_done(sftp, xfer)) {
	    pktin = sftp_recv(sftp);
	    ret = xfer_upload_gotpkt(sftp, xfer, pktin);
	    if (ret <= 0 && !err) {
		sftp_log("error while writing: %s\n", fxp_error(sftp));
		err = 1;
	    }
	}
    }
...
SftpWildcardMatcher *sftp_begin_wildcard_matching(sftp_handle* sftp, char *name)
{
    struct sftp_packet *pktin;
    struct sftp_request *req, *rreq;
    char *wildcard;
    ...
    if (!check) {
	sftp_log("Multiple-level wildcards are not supported\n");
	sfree(unwcdir);
	return NULL;
    }

    cdir = canonify(sftp, unwcdir);

    sftp_register(sftp, req = fxp_opendir_send(sftp, cdir));
    rreq = sftp_find_request(sftp, pktin = sftp_recv(sftp));
    assert(rreq == req);
...
char *sftp_wildcard_get_filename(sftp_handle* sftp, SftpWildcardMatcher *swcm)
{
	...
	if (swcm->names && swcm->namepos >= swcm->names->nnames) {
	    fxp_free_names(sftp, swcm->names);
	    swcm->names = NULL;
	}

	if (!swcm->names) {
	    sftp_register(sftp, req = fxp_readdir_send(sftp, swcm->dirh));
	    rreq = sftp_find_request(sftp, pktin = sftp_recv(sftp));
	    assert(rreq == req);
	    swcm->names = fxp_readdir_recv(sftp, pktin, rreq);
...
void sftp_finish_wildcard_matching(sftp_handle* sftp, SftpWildcardMatcher *swcm)
{
    struct sftp_packet *pktin;
    struct sftp_request *req, *rreq;

    sftp_register(sftp, req = fxp_close_send(sftp, swcm->dirh));
    rreq = sftp_find_request(sftp, pktin = sftp_recv(sftp));
    assert(rreq == req);
    fxp_close_recv(sftp, pktin, rreq);
 ...
 */
int wildcard_iterate(sftp_handle* sftp, char *filename, int (*func)(sftp_handle*, void *, char *), void *ctx)
{
    ...
    if (is_wc) {
	SftpWildcardMatcher *swcm = sftp_begin_wildcard_matching(sftp, filename);
	int matched = FALSE;
	sfree(unwcfname);
	...
	while ( (newname = sftp_wildcard_get_filename(sftp, swcm)) != NULL ) {
	    cname = canonify(sftp, newname);
	    if (!cname) {
		sftp_log("%s: canonify: %s\n", newname, fxp_error(sftp));
		ret = 0;
	    }
	    matched = TRUE;
	    ret &= func(sftp, ctx, cname);
	    sfree(cname);
	}

	if (!matched) {
	    /* Politely warn the user that nothing matched. */
	    sftp_log("%s: nothing matched\n", filename);
	}

	sftp_finish_wildcard_matching(sftp, swcm);
    } else {
	cname = canonify(sftp, unwcfname);
	if (!cname) {
	    sftp_log("%s: canonify: %s\n", filename, fxp_error(sftp));
	    ret = 0;
	}
	ret = func(sftp, ctx, cname);
	sfree(cname);
...
int sftp_cmd_null(sftp_handle* sftp, struct sftp_command *cmd)
{
    return 1;			       /* success */
}

int sftp_cmd_unknown(sftp_handle* sftp, struct sftp_command *cmd)
{
    sftp_log("psftp: unknown command \"%s\"\n", cmd->words[0]);
    return 0;			       /* failure */
}

int sftp_cmd_quit(sftp_handle* sftp, struct sftp_command *cmd)
{
    return -1;
}

int sftp_cmd_close(sftp_handle* sftp, struct sftp_command *cmd)
{
    if (sftp->back == NULL) {
	not_connected(sftp);
	return 0;
...
int sftp_cmd_ls(sftp_handle* sftp, struct sftp_command *cmd)
{
	...
	while (1) {
		...
		if (fxp_error_type(sftp) == SSH_FX_EOF)
		    break;
		sftp_log("Reading directory %s: %s\n", dir, fxp_error(sftp));
		break;
	    }
	    if (names->nnames == 0) {
		fxp_free_names(sftp, names);
		...
		if (!wildcard || wc_match(wildcard, names->names[i].filename))
		    ournames[nnames++] = fxp_dup_name(sftp, &names->names[i]);
...
int sftp_cmd_cd(sftp_handle* sftp, struct sftp_command *cmd)
{
    ...
    sftp_register(sftp, req = fxp_opendir_send(sftp, dir));
    rreq = sftp_find_request(sftp, pktin = sftp_recv(sftp));
    assert(rreq == req);
    dirh = fxp_opendir_recv(sftp, pktin, rreq);

    if (!dirh) {
	sftp_log("Directory %s: %s\n", dir, fxp_error(sftp));
	sfree(dir);
	return 0;
    }
    ...
    sfree(sftp->pwd);
    sftp->pwd = dir;
    sftp_log("Remote directory is now %s\n", sftp->pwd);
 ...
 */
int sftp_cmd_pwd(sftp_handle* sftp, struct sftp_command *cmd)
{
    if (sftp->back == NULL) {
	not_connected(sftp);
	return 0;
    ...
    sftp_log("Remote directory is %s\n", sftp->pwd);
    return 1;
}
 ...
 */
int sftp_general_get(sftp_handle* sftp, struct sftp_command *cmd, int restart, int multiple)
{
	...
	if (!strcmp(cmd->words[i], "--")) {
	    ...
	    sftp_log("%s: unrecognised option '%s'\n", cmd->words[0], cmd->words[i]);
	    return 0;
	}
	i++;
    }
	...
	if (multiple && !wc_unescape(unwcfname, origfname)) {
	    swcm = sftp_begin_wildcard_matching(sftp, origfname);
	    if (!swcm) {
		sfree(unwcfname);
		continue;
	    }
	    origwfname = sftp_wildcard_get_filename(sftp, swcm);
	    if (!origwfname) {
		/* Politely warn the user that nothing matched. */
		sftp_log("%s: nothing matched\n", origfname);
		sftp_finish_wildcard_matching(sftp, swcm);
	...
	while (origwfname) {
	    fname = canonify(sftp, origwfname);

	    if (!fname) {
		sftp_log("%s: canonify: %s\n", origwfname, fxp_error(sftp));
		sfree(unwcfname);
		return 0;
	    }
	    ...
	    ret = sftp_get_file(sftp, fname, outfname, recurse, restart);
...
int sftp_cmd_get(sftp_handle* sftp, struct sftp_command *cmd)
{
    return sftp_general_get(sftp, cmd, 0, 0);
}
int sftp_cmd_mget(sftp_handle* sftp, struct sftp_command *cmd)
{
    return sftp_general_get(sftp, cmd, 0, 1);
}
int sftp_cmd_reget(sftp_handle* sftp, struct sftp_command *cmd)
{
    return sftp_general_get(sftp, cmd, 1, 0);
}
 ...
 * can handle wildcards.
 */
int sftp_general_put(sftp_handle* sftp, struct sftp_command *cmd, int restart, int multiple)
{
	...
	while (wfname) {
	    ...
	    outfname = canonify(sftp, origoutfname);
	    if (!outfname) {
		sftp_log("%s: canonify: %s\n", origoutfname, fxp_error(sftp));
		if (wcm) {
		    sfree(wfname);
		    finish_wildcard_matching(wcm);
		}
	    ...
	    ret = sftp_put_file(sftp, wfname, outfname, recurse, restart);
	    sfree(outfname);
...
int sftp_cmd_put(sftp_handle* sftp, struct sftp_command *cmd)
{
    return sftp_general_put(sftp, cmd, 0, 0);
}
int sftp_cmd_mput(sftp_handle* sftp, struct sftp_command *cmd)
{
    return sftp_general_put(sftp, cmd, 0, 1);
}
int sftp_cmd_reput(sftp_handle* sftp, struct sftp_command *cmd)
{
    return sftp_general_put(sftp, cmd, 1, 0);
}

int sftp_cmd_mkdir(sftp_handle* sftp, struct sftp_command *cmd)
{
    char *dir;
    struct sftp_packet *pktin;
    struct sftp_request *req, *rreq;
    ...
    if (cmd->nwords < 2) {
	sftp_log("mkdir: expects a directory\n");
	return 0;
    }

    ret = 1;
    for (i = 1; i < cmd->nwords; i++) {
	dir = canonify(sftp, cmd->words[i]);
	if (!dir) {
	    sftp_log("%s: canonify: %s\n", dir, fxp_error(sftp));
	    return 0;
	}

	sftp_register(sftp, req = fxp_mkdir_send(sftp, dir));
	rreq = sftp_find_request(sftp, pktin = sftp_recv(sftp));
	assert(rreq == req);
	result = fxp_mkdir_recv(sftp, pktin, rreq);
...
static int sftp_action_rmdir(sftp_handle* sftp, void *vctx, char *dir)
{
    struct sftp_packet *pktin;
    struct sftp_request *req, *rreq;
    int result;

    sftp_register(sftp, req = fxp_rmdir_send(sftp, dir));
    rreq = sftp_find_request(sftp, pktin = sftp_recv(sftp));
    assert(rreq == req);
    result = fxp_rmdir_recv(sftp, pktin, rreq);
...
int sftp_cmd_rmdir(sftp_handle* sftp, struct sftp_command *cmd)
{
    int i, ret;
    ...
    if (cmd->nwords < 2) {
	sftp_log("rmdir: expects a directory\n");
	return 0;
    }

    ret = 1;
    for (i = 1; i < cmd->nwords; i++)
	ret &= wildcard_iterate(sftp, cmd->words[i], sftp_action_rmdir, NULL);

    return ret;
}

static int sftp_action_rm(sftp_handle* sftp, void *vctx, char *fname)
{
    struct sftp_packet *pktin;
    struct sftp_request *req, *rreq;
    int result;

    sftp_register(sftp, req = fxp_remove_send(sftp, fname));
    rreq = sftp_find_request(sftp, pktin = sftp_recv(sftp));
    assert(rreq == req);
    result = fxp_remove_recv(sftp, pktin, rreq);
...
int sftp_cmd_rm(sftp_handle* sftp, struct sftp_command *cmd)
{
    int i, ret;
    ...
    if (cmd->nwords < 2) {
	sftp_log("rm: expects a filename\n");
	return 0;
    }

    ret = 1;
    for (i = 1; i < cmd->nwords; i++)
	ret &= wildcard_iterate(sftp, cmd->words[i], sftp_action_rm, NULL);

    return ret;
}

static int check_is_dir(sftp_handle* sftp, char *dstfname)
{
    struct sftp_packet *pktin;
    struct sftp_request *req, *rreq;
    struct fxp_attrs attrs;
    ...
    sftp_register(sftp, req = fxp_stat_send(sftp, dstfname));
    rreq = sftp_find_request(sftp, pktin = sftp_recv(sftp));
    assert(rreq == req);
    result = fxp_stat_recv(sftp, pktin, rreq, &attrs);
...
static int sftp_action_mv(sftp_handle* sftp, void *vctx, char *srcfname)
{
	...
	newcanon = canonify(sftp, newname);
	if (!newcanon) {
	    sftp_log("%s: canonify: %s\n", newname, fxp_error(sftp));
	    sfree(newname);
	    return 0;
    ...
    sftp_register(sftp, req = fxp_rename_send(sftp, srcfname, finalfname));
    rreq = sftp_find_request(sftp, pktin = sftp_recv(sftp));
    assert(rreq == req);
    result = fxp_rename_recv(sftp, pktin, rreq);

    error = result ? NULL : fxp_error(sftp);
...
int sftp_cmd_mv(sftp_handle* sftp, struct sftp_command *cmd)
{
    ...
    if (cmd->nwords < 3) {
	sftp_log("mv: expects two filenames\n");
	return 0;
    }

    ctx->dstfname = canonify(sftp, cmd->words[cmd->nwords-1]);
    if (!ctx->dstfname) {
	sftp_log("%s: canonify: %s\n", ctx->dstfname, fxp_error(sftp));
	return 0;
    }
     ...
     */
    ctx->dest_is_dir = check_is_dir(sftp, ctx->dstfname);
    if ((cmd->nwords > 3 || is_wildcard(cmd->words[1])) && !ctx->dest_is_dir) {
	sftp_log("mv: multiple or wildcard arguments require the destination"
	       " to be a directory\n");
	sfree(ctx->dstfname);
	return 0;
    }
	...
	ret &= wildcard_iterate(sftp, cmd->words[i], sftp_action_mv, ctx);
...
static int sftp_action_chmod(sftp_handle* sftp, void *vctx, char *fname)
{
    struct fxp_attrs attrs;
    struct sftp_packet *pktin;
    struct sftp_request *req, *rreq;
	...
	sftp_log("get attrs for %s: %s\n", fname,
	       result ? "file permissions not provided" : fxp_error(sftp));
	return 0;
    }
    ...
    sftp_register(sftp, req = fxp_setstat_send(sftp, fname, attrs));
    rreq = sftp_find_request(sftp, pktin = sftp_recv(sftp));
    assert(rreq == req);
    result = fxp_setstat_recv(sftp, pktin, rreq);

    if (!result) {
	sftp_log("set attrs for %s: %s\n", fname, fxp_error(sftp));
	return 0;
    }
...
int sftp_cmd_chmod(sftp_handle* sftp, struct sftp_command *cmd)
{
    char *mode;
    int i, ret;
    ...
    if (cmd->nwords < 3) {
	sftp_log("chmod: expects a mode specifier and a filename\n");
	return 0;
    }
	...
	if (mode[strspn(mode, "01234567")]) {
	    sftp_log("chmod: numeric file modes should"
	...
	while (*mode) {
		...
		switch (*mode) {
		  ...
		  default:
		    sftp_log("chmod: file mode '%.*s' contains unrecognised"
	    ...
	    if (!*mode || *mode == ',') {
		sftp_log("chmod: file mode '%.*s' is incomplete\n",
		...
		switch (*mode) {
		  ...
		  case 's':
			...
			(subset & 06777) != 02070) {
			sftp_log("chmod: file mode '%.*s': set[ug]id bit should"
		...
		sftp_log("chmod: file mode '%.*s' contains no user/group/other"
	...
	ret &= wildcard_iterate(sftp, cmd->words[i], sftp_action_chmod, ctx);

    return ret;
}

static int sftp_cmd_open(sftp_handle* sftp, struct sftp_command *cmd)
{
    int portnumber;

    if (sftp->back != NULL) {
	sftp_log("psftp: already connected\n");
	return 0;
    }

    if (cmd->nwords < 2) {
	sftp_log("open: expects a host name\n");
	return 0;
    }
	...
	if (portnumber == 0) {
	    sftp_log("open: invalid port number\n");
	    return 0;
	}
    } else
	portnumber = 0;
...
static int sftp_cmd_lcd(sftp_handle* sftp, struct sftp_command *cmd)
{
    char *currdir, *errmsg;

    if (cmd->nwords < 2) {
	sftp_log("lcd: expects a local directory name\n");
	return 0;
    }

    errmsg = psftp_lcd(cmd->words[1]);
    if (errmsg) {
	sftp_log("lcd: unable to change directory: %s\n", errmsg);
	sfree(errmsg);
	return 0;
    }

    currdir = psftp_getcwd();
    sftp_log("New local directory is %s\n", currdir);
    sfree(currdir);

    return 1;
}

static int sftp_cmd_lpwd(sftp_handle* sftp, struct sftp_command *cmd)
{
    char *currdir;

    currdir = psftp_getcwd();
    sftp_log("Current local directory is %s\n", currdir);
    sfree(currdir);

    return 1;
}

static int sftp_cmd_pling(sftp_handle* sftp, struct sftp_command *cmd)
{
    int exitcode;
...
static int sftp_cmd_help(sftp_handle* sftp, struct sftp_command *cmd);
...
static struct sftp_cmd_lookup {
    ...
    char *shorthelp;
    char *longhelp;
    int (*obey) (sftp_handle* sftp, struct sftp_command *);
} sftp_lookup[] = {
    /*
...
static int sftp_cmd_help(sftp_handle* sftp, struct sftp_command *cmd)
{
	...
	for (i = 0; i < sizeof(sftp_lookup) / sizeof(*sftp_lookup); i++) {
	    ...
	    sftp_log("%-*s", maxlen+2, lookup->name);
	...
	for (i = 1; i < cmd->nwords; i++) {
	    ...
	    if (!lookup) {
		sftp_log("help: %s: command not found\n", cmd->words[i]);
	    } else {
		sftp_log("%s", lookup->name);
		if (lookup->longhelp == NULL)
		    lookup = lookup_command(lookup->shorthelp);
		sftp_log("%s", lookup->longhelp);
	    }
	}
    }
    return 1;
...
struct sftp_command *sftp_findcmd(sftp_handle* sftp, char* const cmdline)
{
...
struct sftp_command *sftp_getcmd(sftp_handle* sftp, FILE *fp, int mode, int modeflags)
{
    char *line;
	...
	line = ssh_sftp_get_cmdline("psftp> ", sftp->back == NULL);
    }
    ...
    struct sftp_command *cmd = sftp_findcmd(sftp, line);
...
static int do_sftp_init(sftp_handle* sftp)
{
	...
	sftp_log("Remote working directory is %s\n", sftp->homedir);
    }
    sftp->pwd = dupstr(sftp->homedir);
    return 0;
}

void do_sftp_cleanup(sftp_handle* sftp)
{
    char ch;
    if (sftp->back) {
	sftp->back->special(sftp->backhandle, TS_EOF);
...
void do_sftp(sftp_handle* sftp, int mode, int modeflags, char *batchfile)
{
    FILE *fp;
    int ret;
    ...
    if (mode == 0) {
        ...
        while (1) {
	    struct sftp_command *cmd;
	    cmd = sftp_getcmd(sftp, NULL, 0, 0);
	    if (!cmd)
		break;
	    ret = cmd->obey(sftp, cmd);
	    if (cmd->words) {
		int i;
        ...
        if (!fp) {
	    sftp_log("Fatal: unable to open %s\n", batchfile);
	    return;
        }
        while (1) {
	    struct sftp_command *cmd;
	    cmd = sftp_getcmd(sftp, fp, mode, modeflags);
	    if (!cmd)
		break;
	    ret = cmd->obey(sftp, cmd);
	    if (ret < 0)
 ...
 */
static void usage(sftp_handle* sftp)
{
    sftp_log("PuTTY Secure File Transfer (SFTP) client\n");
    sftp_log("%s\n", ver);
    sftp_log("Usage: psftp [options] [user@]host\n");
    sftp_log("Options:\n");
    sftp_log("  -V        print version information and exit\n");
    sftp_log("  -pgpfp    print PGP key fingerprints and exit\n");
    sftp_log("  -b file   use specified batchfile\n");
    sftp_log("  -bc       output batchfile commands\n");
    sftp_log("  -be       don't stop batchfile processing if errors\n");
    sftp_log("  -v        show sftp->verbose messages\n");
    sftp_log("  -load sessname  Load settings from saved session\n");
    sftp_log("  -l user   connect with specified username\n");
    sftp_log("  -P port   connect to specified port\n");
    sftp_log("  -pw passw login with specified password\n");
    sftp_log("  -1 -2     force use of particular SSH protocol version\n");
    sftp_log("  -4 -6     force use of IPv4 or IPv6\n");
    sftp_log("  -C        enable compression\n");
    sftp_log("  -i key    private key file for authentication\n");
    sftp_log("  -noagent  disable use of Pageant\n");
    sftp_log("  -agent    enable use of Pageant\n");
    sftp_log("  -batch    disable all interactive prompts\n");
    cleanup_exit(1);
}

static void version(sftp_handle* sftp)
{
  sftp_log("psftp: %s\n", ver);
  cleanup_exit(1);
}
...
static int psftp_connect(sftp_handle* sftp, char *userhost, char *user, int portnumber)
{
	...
	*host++ = '\0';
	if (user) {
	    sftp_log("psftp: multiple usernames specified; using \"%s\"\n",
		   user);
	} else
	    user = userhost;
    }
...
int psftp_main(int argc, char *argv[])
{
    ...
    if (userhost) {
	int ret;
	ret = psftp_connect(sftp, userhost, user, portnumber);
	sfree(userhost);
	if (ret)
	    return 1;
	if (do_sftp_init(sftp))
	    return 1;
    } else {
	sftp_log("psftp: no hostname specified; use \"open host.name\""
	       " to connect\n");
    }
