				 ...
				 void *data, int event)
{
	...
	if (!ssd->midsession &&
	    ...
	    int isdef = !strcmp(savedsession, DEFAULT_SESSION_NAME);
	    if (!savedsession[0]) {
		int i = dlg_listbox_index(ssd->listbox, dlg);
		if (i < 0) {
		    dlg_beep(dlg);
		      ...
		      int protocol, int protcfginfo)
{
    ...
    ctrl_text(s, "(Log file name can contain &Y, &M, &D for date,"
	      " &T for time, &S for session name, &H for host name"
	      " , and &P for the desktop path)",
	      HELPCTX(logging_filename));
    ...
    ctrl_editbox(s, "Lines at a scroll(-1:half a screen, -2:a screen)", 'l', 15,
		 HELPCTX(window_scrollback),
		 dlg_stdeditbox_handler, I(offsetof(Config,scrolllines)), I(-1));
